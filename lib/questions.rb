...
def separate_array_into_even_and_odd_numbers(array)
  a = []
  a.push(array.select{ |x| x.even? }, array.select{ |x| x.odd? })
end
...
def remove_capital_letters_from_string(string)
  regex = /[A-Z]/
  string.split("").reject{ |x| regex.match(x) }.join
end
...
def titleize_a_string(string)
  lowercase = ['a', 'and', 'the']
  string.split.each_with_index{ |word, i| word.capitalize! unless (lowercase.include?(word) && i != 0) }.join(' ')
end
...
def is_a_3_dot_range?(range)
  range.to_a != (range.first...(range.last+1)).to_a
end
...
def your_birthday_is_on_a_friday_in_the_year(birthday)
  ...
  y = birthday.year
  loop do
    future_birthday = Time.new(y, m, d)
    return y if future_birthday.wday === 5
    y += 1
  end
end
...
def count_words_of_each_length_in_a_file(file_path)
  file = File.read(file_path)
  file_words = file.split(' ').map{|word| word.match(/\w*/).to_s }
  word_length_count = Hash.new(0)
  file_words.each{ |word| word_length_count[word.length] += 1 }
  word_length_count
end
